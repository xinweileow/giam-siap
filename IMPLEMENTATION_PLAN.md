...
**Not yet done**: Hermes/Telegram wiring (§7 step 3, including actually wiring the watcher's `checkNow()`/`onAlert` hooks to Telegram), the dashboard (§7 step 5), and the zkLogin swap-in (§7 step 6) — those remain exactly as described below. The mock vendor site is still teammates' open item (§7 step 2b) — a dev-only stand-in vendor keypair is registered in `VendorRegistry` today, and `agent/watcher/dev/vendor-stub.ts` (above) lets the watcher be tested against it before their real endpoint exists.
