...
**Not yet done**: closing the two gaps surfaced by the MCP/NL-parsing session above (automatic Telegram-side context injection, and a real signer for `createOrder`'s output until zkLogin lands), wiring the watcher's `checkNow()`/`onAlert` hooks to Telegram (§7 step 3), and the zkLogin swap-in (§7 step 6). The dashboard (§7 step 5) is done, per the update just above. The mock vendor site is still teammates' open item (§7 step 2b) — a dev-only stand-in vendor keypair is registered in `VendorRegistry` today, and `agent/watcher/dev/vendor-stub.ts` lets the watcher be tested against it before their real endpoint exists.
