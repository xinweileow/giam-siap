...
**Not yet done**: closing the two gaps just above, the dashboard (§7 step 5, being built in parallel on `task/dashboard`), and the zkLogin swap-in (§7 step 6). The mock vendor site is still teammates' open item (§7 step 2b) — a dev-only stand-in vendor keypair is registered in `VendorRegistry` today, and `agent/watcher/dev/vendor-stub.ts` lets the watcher be tested against it before their real endpoint exists.
