...
**Update (same day, later session again) — §7 step 3's MCP connection is live and NL parsing is validated.** Two real gaps found and fixed along the way, worth knowing about:
- Both `agent/sui-mcp/src/config.ts` and `agent/watcher/src/config.ts` now self-load their own `.env` via `dotenv`, resolved relative to the source file's own location (not `process.cwd()`). Before this, both required the caller to have already sourced `.env` into the shell — fine for `e2e-smoke.ts` run by hand, but `hermes mcp add --command node --args ...` launches `node dist/server.js` as its own subprocess with none of that; without self-loading, every tool call would have failed with `Missing required env var`. `dotenv` never overrides a var already set in the environment, so this is additive, not a behavior change for anything that already worked. `agent/watcher/src/config.ts`'s `vendorPubkeyHex` now also falls back to sui-mcp's `DEV_VENDOR_PUBKEY_HEX` when `VENDOR_PUBKEY_HEX` isn't set, so local dev works with only `agent/sui-mcp/.env` populated.
- `hermes mcp add sui-tools --command node --args <absolute path to agent/sui-mcp/dist/server.js>` is registered and connects cleanly — `hermes mcp test sui-tools` confirms all 6 tools discovered. One real operational snag: the CLI's "Enable all N tools?" confirmation prompt reads from stdin, and a non-TTY/backgrounded shell hits immediate EOF and auto-cancels before saving anything — piping an answer in (`"y" | hermes mcp add ...`) works around this; a real interactive terminal wouldn't hit it at all. Also confirmed: this machine's Hermes install already has Telegram/Discord/WhatsApp gateway credentials configured and the gateway service running from prior, unrelated setup — so §7 step 3's "hermes gateway setup for Telegram/BotFather" sub-step was already satisfied coming into this session, nothing to do there.
- NL intent parsing → `createOrder` tool-call correctness (§8.3's first checklist item) is now verified, not just assumed: ran the plan's exact demo phrasing ("Procure 50kg Coffee Beans with a max budget of $500 at target price $10/kg") through `hermes -z` (oneshot mode) with the owner address, `rate_mist_per_cent`, and the item's vendor URL supplied as context, **5 times in a row** — every run produced the identical, correct call: `itemId="coffee"`, `targetPriceCents=1000`, `quantity=50`, `paymentAmountMist=50000000` (matching the escrow-sufficiency invariant exactly), and the tool successfully returned unsigned tx bytes. §8.3's "5 times in a row" requirement is met.
- **Two real gaps surfaced by this test, not yet solved** (see new TODOS.md items): (1) in production, Hermes needs this same context — the owner's address, the on-chain rate, and each item's vendor URL — available to it automatically inside a live Telegram conversation, not hand-fed in a one-off test prompt. §4.1 names `agent/hermes.config/` for this but that directory doesn't exist yet, and it's not yet confirmed whether the Telegram gateway's conversations actually run with this repo as their working directory (which is what would make an `AGENTS.md` here auto-inject, per Hermes's own `--ignore-rules` help text). (2) `createOrder` only ever returns unsigned tx bytes — by design, per §4.5, since only the owner's browser should sign. But the real zkLogin browser-signing flow (§7 step 6) doesn't exist yet, and unlike `e2e-smoke.ts` (which signs inline in the same script), nothing in the Hermes-triggered path today can sign and submit that transaction with the §3-step-5 stand-in keypair. Until one of these two is built, a live Telegram message can get as far as "here is an unsigned transaction" and no further — it cannot yet actually lock funds end-to-end the way §7 step 3's last sentence describes.
...
**Update (2026-09-04) — the four items below are now closed, except real zkLogin itself (which is genuinely blocked on external credentials, not on code).**

- **Hermes's automatic Telegram-side context** (§4.1's `agent/hermes.config/`, TODOS.md's top P0 item) is built. `agent/hermes.config/` is now a real, repo-tracked directory: `context.json` (owner address, `rate_mist_per_cent`, the item→vendor-URL table, the watcher's check-now URL — the single source of truth), `system-prompt.md` (a `{{PLACEHOLDER}}` template for the prose/instructions), and `sync-to-hermes-profile.mjs`, which renders both into the actual live mechanism: a Telegram channel's `system_prompt` override inside the `giam-siap` Hermes profile's `gateway-config.yaml` (`%LOCALAPPDATA%\hermes\profiles\giam-siap\` on this machine) — confirmed to already exist and already be populated correctly from an earlier session, just not repo-tracked or regenerable until now. Investigated and answered the open question from the prior session: a live Telegram gateway conversation does **not** reliably run with this repo as its working directory (profiles are independent, out-of-repo, per-machine state — `terminal.cwd` in this profile's `config.yaml` is just `.`), so a repo-local `AGENTS.md` alone would not have been sufficient; the channel-override `system_prompt` is the mechanism that's actually proven to work (see `agent/hermes.config/README.md` for the full reasoning). A repo-root `AGENTS.md` was added anyway, since it's genuinely useful for `hermes -z`/`hermes chat` sessions run from this repo (which is how §8.3's NL-parsing check was validated last session). The item table now has three items instead of one: `coffee` (the working signed dev-vendor-stub endpoint, plus a `displayUrl` pointing at teammates' real `cooking-bistro.vercel.app` mock vendor site — see below), and two new context-only items (`sugar`, `rice`) pointing at real, unrelated wholesale vendor pages for conversational realism, per this session's instructions — neither has a signed price API, so the watcher will safely alert-and-never-settle if an order is ever created against them (§9.1), which `system-prompt.md` tells Hermes to disclose to the owner up front.
- **Teammates' mock vendor site exists now**: `https://cooking-bistro.vercel.app/` — a real, polished B2B wholesale catalog page (coffee, milk, butter, matcha), confirmed reachable. It does **not** yet expose the signed `GET /api/price?item=<id>` endpoint from §4.2's spec (every path tried 404s) — it's the display/UI half of their deliverable, not the signed-quote half. `checkVendorPrice`/the watcher still point `coffee` at the local `agent/watcher/dev/vendor-stub.ts` stand-in for actual signed price verification; `cooking-bistro.vercel.app` is wired in as a `displayUrl` (conversational context only) until they add the real endpoint and hand over their signing pubkey (§7 step 2b).
- **A dev-only signer bridge for `createOrder`/`cancelOrder`'s unsigned output** (TODOS.md's other P0 item) is built: `agent/sui-mcp/src/tools/devSignAndSubmit.ts` exports `devSignAndSubmitTx(client, signer, {unsignedTxBytesBase64})`, which does exactly what a real `/sign` page will eventually do — `signer.signTransaction(bytes)` then `client.executeTransaction({transaction, signatures, include})` — registered as a new MCP tool (`devSignAndSubmitTx`) signing with the same dev stand-in keypair as `AGENT_PRIVATE_KEY`. `agent/hermes.config/system-prompt.md` now instructs Hermes to call it immediately after `createOrder`/`cancelOrder`, while explicitly telling the owner this is a temporary dev stand-in, not real zkLogin. This is what actually closes the gap the prior session flagged: a live Telegram order request can now go all the way to a real, on-chain `Locked` order, not just an unsigned-tx dead end. 1 new Vitest test (mocked signer/client — 14/14 pass in `agent/sui-mcp`, up from 13).
- **The watcher's `checkNow()`/`onAlert` hooks are wired to Telegram** (§7 step 3, §9.1). `agent/watcher/src/httpServer.ts` exposes a loopback-only `POST /check-now` (default port 4300, configurable via `WATCHER_HTTP_PORT`) that runs the exact same deterministic `tick()` as the scheduled timer — `agent/hermes.config/system-prompt.md` tells Hermes to hit it with `curl` via the terminal tool when the owner asks to "check now", satisfying §4.3 point 2 without adding any LLM call to the settlement path itself. `agent/watcher/src/telegram.ts` posts real Telegram messages by shelling out to `hermes send --to telegram` (discovered via `hermes send --help`: reuses the gateway's platform credentials directly, no LLM, no running gateway required) against the `giam-siap` Hermes profile, which already has its own Telegram bot token and home chat id configured. `onAlert` (previously `console.error`-only) now also sends a real self-alert message; a new notification fires on every `executed` outcome with a Suiscan link, satisfying §8.3's "settlement confirmation with a working Suiscan link" checklist item. A notification failure is deliberately fire-and-forget and never affects what the watcher already decided on-chain (§4.1's determinism note applies to side effects too). 2 new Vitest tests (mocked `child_process` — 14/14 pass in `agent/watcher`, up from 12).
- **zkLogin (Enoki) is scaffolded, not wired live** — this genuinely needs external credentials (an Enoki API key from `portal.enoki.mystenlabs.com`, a Google OAuth client id) that only the project owner can create, so it's untestable this session regardless of how much code exists. What's built: `dashboard/src/app/auth/page.tsx` (`?telegramUserId=<id>` — Google OAuth via Enoki, auto-faucet, persists `{telegramUserId → address}`) and `dashboard/src/app/sign/page.tsx` (`?tx=<pendingId>` — restores the session with no repeat Google prompt, signs+submits the real per-order transaction), backed by two small JSON-file stores (`dashboard/src/lib/ownerSessions.ts`, `dashboard/src/lib/pendingTx.ts`, both under a configurable `DASHBOARD_DATA_DIR` so they survive a restart, §8.4's checklist item) and new `/api/owner-session`, `/api/faucet`, `/api/pending-tx[/[id]]` routes. **A real, deliberate architecture decision worth knowing about**: this uses `@mysten/enoki`'s `EnokiFlow`/`EnokiKeypair` classes directly, not the currently-recommended `registerEnokiWallets` + `@mysten/dapp-kit` integration — `@mysten/dapp-kit@1.1.17` prints its own deprecation warning on install ("only supports the deprecated Sui JSON-RPC API"), i.e. it's built on the exact `SuiClient` this project already migrated off of everywhere else after the public testnet fullnode removed the JSON-RPC method its tx builder depended on (see this doc's SDK-migration finding above). `EnokiFlow` has no such dependency and hands back an `EnokiKeypair` that implements the same `Signer` interface every other keypair in this codebase does, so it drops directly into `SuiGrpcClient` calls — see `dashboard/src/lib/enokiFlow.ts`'s comment for the full reasoning. **Known gap, stated honestly**: `/sign` submits with the owner paying their own (fauceted) gas; it does not yet call Enoki's gas-station sponsorship API, which needs a private `ENOKI_SECRET_KEY` on a backend route (§4.5's "owner never touches gas" promise isn't fully closed — see TODOS.md). Also not wired: `agent/sui-mcp`'s `createOrder`/`cancelOrder` still hand Hermes raw unsigned bytes today (signed by the dev bridge above), rather than POSTing to `/api/pending-tx` and handing back a `/sign` link — that swap is the actual last step of §7 step 6, deliberately left undone until real credentials make it testable.

The dashboard (§7 step 5) is done, per the earlier update above.

**Update (2026-09-05) — real zkLogin is wired live; §7 step 6 is now fully closed except gas sponsorship.** The project owner created a real Enoki account and Google OAuth client this session (external credentials that were the last real blocker on §7 step 6). With those in `dashboard/.env`, `/auth` was verified end-to-end in a real browser: Google login → real zkLogin Sui testnet address → auto-faucet, no errors. One real Enoki-side setup gap surfaced and got fixed along the way, worth knowing if this is ever redone from scratch: `createAuthorizationURL` returns a 400 until the Google OAuth Client ID is *also* registered as a trusted Auth Provider inside the Enoki Portal itself (Portal → App → Auth Providers → Google) — having a valid Google client id and the network enabled on the API key is not sufficient on its own; Enoki validates the client id against what's explicitly registered per-app.

With `/auth` proven, the actual remaining wiring (TODOS.md's P1 item) got built: `agent/sui-mcp` gained a `requestOwnerSignature` tool (`src/tools/requestOwnerSignature.ts`) that POSTs `createOrder`/`cancelOrder`'s unsigned tx bytes to the dashboard's `POST /api/pending-tx` and returns a `/sign?tx=<id>` link, instead of the dev-signer bridge (`devSignAndSubmitTx`) signing immediately. `agent/hermes.config/system-prompt.md` now instructs Hermes to call `requestOwnerSignature` and send the owner that link as the default behavior — re-synced to the live `giam-siap` Hermes profile. 2 new Vitest tests (16/16 pass in `agent/sui-mcp`, up from 14); `agent/watcher` unaffected (still 14/14) since it only depends on sui-mcp's other tool exports. `devSignAndSubmitTx` was deliberately **kept, not deleted** as TODOS.md originally planned — it's now clearly relabeled in its own tool description as an explicit, opt-in-only fallback rather than removed, since keeping a working fallback around is cheap insurance (§9.2's own demo-day philosophy) and it can still be deleted later once the real flow has more runs behind it.

**Still open, stated honestly**: gas sponsorship. `/sign` still submits with the owner paying their own fauceted testnet gas, not Enoki-sponsored gas — that needs `EnokiClient.createSponsoredTransaction`/`executeSponsoredTransaction` behind a new backend route holding the private `ENOKI_SECRET_KEY` (which exists, from the same Enoki account, but isn't in any `.env` yet since no code path uses it). §4.5's "owner never touches gas" promise is now half true: true for the *first* balance (auto-faucet on `/auth`), not yet true for every order's gas after that. Tracked in TODOS.md as its own item, not yet built.

**Update (2026-09-05, later same session) — Hermes now uses the owner's real address, and /auth+/sign flow into each other instead of dead-ending.** Testing the flow above surfaced a real gap immediately: `requestOwnerSignature` registers whatever `ownerAddress` Hermes passes to `createOrder`, but the system prompt still hardcoded the dev stand-in address there — meaning `/sign` would always refuse to submit (it checks the signed-in Enoki address against the order's recorded owner) since nobody's real zkLogin address is the dev stand-in address. Fixed: `agent/sui-mcp` gained a `getOwnerAddress` tool (`src/tools/getOwnerAddress.ts`, 3 new Vitest tests — 19/19 pass, up from 16) that reads the dashboard's `/api/owner-session?telegramUserId=...` (already built, just unused until now). `system-prompt.md` now has Hermes call this *before every* `createOrder`/`cancelOrder` — never reusing an address from earlier in the conversation — and if it comes back null (owner hasn't signed in), send them the `/auth` link and hold off on building the order at all, rather than guessing. `context.json` gained `dashboardUrl`; the owner's Telegram user id (already tracked as `telegram.homeChatId`, since this is a single-owner deployment, §0.5) is now also substituted into the prompt as `{{OWNER_TELEGRAM_USER_ID}}`.

Separately, `/auth` and `/sign` were unified into one flow instead of two dead-ending pages, per direct feedback that tapping a sign-in link shouldn't strand the owner: `/sign` now redirects to `/auth?returnTo=<its own URL>` the moment it finds no active browser session (via `window.location.href`, with `returnTo` stashed in `localStorage` the same way `telegramUserId` already was, since Google's OAuth redirect strips query params and returns state in the URL hash instead) — so a re-login round-trips straight back to finish approving the specific transaction, instead of leaving the owner to manually reopen a link. `/auth`, when visited with nothing to return to, now also shows the signed-in owner's own orders (fetched client-side from the existing `/api/state`, filtered by address) with a status badge per order and a plain reminder to message the bot for anything new — replacing the old bare "you're signed in, close this tab" dead end. Verified in-browser (not just built): registered a real pending-tx record and confirmed `/sign` correctly redirects to `/auth` with the return URL intact in both the address bar and `localStorage`, with no console errors — the only piece not verifiable without a real Google account completing OAuth is the final "lands back on /sign automatically" hop itself.

**Update (2026-09-05, same session) — a real bug caught before it could bite on the actual "Approve & submit" click.** `/sign/page.tsx`'s `signAndSubmit` called the server-only `loadConfig()` (`dashboard/src/lib/config.ts`, reads plain `SUI_PACKAGE_ID`/`SUI_RPC_URL` via `requireEnv`) directly from browser code — Next.js only exposes `NEXT_PUBLIC_`-prefixed env vars to the client bundle, so this would have thrown `"Missing required env var: SUI_PACKAGE_ID"` the instant anyone clicked Approve, before ever reaching the actual sign/submit logic. Fixed: the client now builds its `SuiGrpcClient` from `getEnokiNetwork()` (already client-safe, reads `NEXT_PUBLIC_SUI_NETWORK`) and a `NEXT_PUBLIC_SUI_RPC_URL`-or-default constant, never importing the server-only config module. Caught by inspection while confirming to the owner whether the button was actually workable — not caught by `npm run build` (TypeScript has no way to know a value will be `undefined` specifically in the browser bundle) or by the earlier smoke tests (both used a deliberately-mismatched `ownerAddress`, so execution never reached the `loadConfig()` line before this fix). Worth remembering: a clean build is not proof a client component's runtime env access is correct — this class of bug needs either a real click-through or a dedicated env-shape check, not just `tsc`.

**A real, separate operational finding from this session, worth recording**: the `giam-siap` Hermes profile was discovered to have inherited stale cross-project state from an earlier, unrelated hackathon project of the owner's (**NGAM**, a 4-agent F&B procurement bot, same owner/Telegram chat id, different repo at `C:\MasterProject\ngam`) — specifically a local `fnb-procurement` skill bundle seeded into the profile's `skills/` directory, and Hermes's own built-in cross-session memory (`profiles/giam-siap/memories/MEMORY.md`/`USER.md`) full of NGAM-specific facts. Because NGAM's domain (F&B procurement, "purchase X kg at target price") is semantically close to Giam Siap's own prompts, the model kept preferring the old NGAM skill/memory content over this profile's own channel system prompt — several live Telegram test runs got redirected into `cd`-ing around the NGAM repo instead of calling `createOrder`. Fixed by: deleting the profile-local `fnb-procurement` skill directory, running `hermes memory reset --yes` to wipe `MEMORY.md`/`USER.md` clean, and separately fixing `profiles/giam-siap/config.yaml`'s `terminal.cwd: .` (a *relative* path, which silently resolved to whatever directory the gateway happened to be launched from — sometimes the NGAM repo) to an absolute `C:\MasterProject\giam-siap`. None of this is a bug in Giam Siap's own code — it's profile-level machine state hygiene — but it's the kind of thing worth checking first if a from-scratch Hermes profile ever starts behaving like it's running a different project.

**Update (2026-09-05, later session) — real Enoki gas sponsorship is wired, closing §4.5's "owner never touches gas" promise, and a genuine payment-source bug it would have caused was caught and fixed first.** Before wiring sponsorship, `agent/sui-mcp/src/tools/createOrder.ts`'s `buildCreateOrderTx` built the escrow payment coin via `tx.splitCoins(tx.gas, ...)` — splitting the escrow amount off the *gas* coin. That was harmless while the owner paid their own gas from the same coin, but the instant `tx.gas` resolves to a sponsor's gas-station coin instead of the owner's, that line would have made Enoki's wallet fund the entire escrow amount, not just network gas — a real bug that would only have shown up as a very confusing production incident. Fixed first: the payment coin is now built via `tx.add(coinWithBalance({ balance: input.paymentAmountMist, useGasCoin: false }))` (`coinWithBalance` from `@mysten/sui/transactions`, confirmed at `2.29.0` to be a real exported intent — `Transaction` even has a `tx.coin({...})` convenience wrapping the same call), sourcing the coin from the owner's own address balance independent of whatever pays gas. `cancelOrder.ts` was checked and confirmed to have no `tx.gas` reference at all — nothing to fix there. `createOrder.test.ts` needed no changes (it only asserts on `MoveCall` shape/argument count, not how the payment argument was constructed) — 19/19 sui-mcp Vitest tests still pass, `npm run build` still clean.

With that fixed, gas sponsorship itself: two new dashboard routes, `POST /api/sponsor-transaction` and `POST /api/sponsor-transaction/execute`, backed by a new `dashboard/src/lib/enokiServer.ts` holding the private `ENOKI_SECRET_KEY` (added to `dashboard/.env`, server-only, never a `NEXT_PUBLIC_` var). The first route takes a registered pending-tx's FULLY-built bytes (gas already resolved to the owner's own coin by `buildCreateOrderTx`/`buildCancelOrderTx` — that step is unchanged), reconstructs them via `Transaction.from()`, re-derives transaction-**kind-only** bytes via `tx.build({client, onlyTransactionKind: true})`, and calls Enoki's real `createSponsoredTransaction` (sender-based — no JWT needed, since the server already knows the owner's address from the pending-tx record) to get back sponsored bytes + a tracking digest. `/sign/page.tsx`'s `signAndSubmit` was rewritten to call this route, sign the *sponsored* bytes it returns with the owner's own `EnokiKeypair` (not the original unsigned bytes), then POST the signature to the second route, which calls `executeSponsoredTransaction({digest, signature})` and marks the pending-tx record submitted/failed accordingly — `/sign` no longer builds or submits anything via `SuiGrpcClient` itself at all, since Enoki now owns execution.

**A real, previously-undocumented Enoki API constraint found by smoke-testing against the live endpoint, not by reading docs**: `createSponsoredTransaction` refuses to sponsor a Move call target it doesn't already know about — `{"code":"invalid_transaction","message":"... is not part of an allow-listed move call target"}`, HTTP 400 — unless the request explicitly lists it via `allowedMoveCallTargets`, or the target is pre-registered in the Enoki Portal's app settings (not done here — only the project owner has portal access). Fixed by deriving `allowedMoveCallTargets` from the transaction's own `MoveCall` commands at request time (`${package}::${module}::${function}`) rather than hardcoding `create_order`/`cancel_order`, so it keeps working if either function ever moves or is renamed. Also confirmed live: Enoki dry-runs the transaction before agreeing to sponsor it, so a real contract-level abort surfaces as an immediate 400 here — caught first-hand when an initial smoke-test payment amount below the on-chain-required escrow correctly came back as `MoveAbort ... abort code: 3` (`E_INSUFFICIENT_ESCROW`), before Enoki would even hand back sponsored bytes.

**What's actually verified vs. not, stated plainly**: `npm run build` is clean for both `agent/sui-mcp` and `dashboard`; all 19 sui-mcp Vitest tests and all 14 watcher Vitest tests still pass (watcher untouched). The real external dependency — Enoki's live `createSponsoredTransaction` API — was smoke-tested directly against production (`https://api.enoki.mystenlabs.com/v1/transaction-blocks/sponsor`) using the real `ENOKI_SECRET_KEY`, a real `create_order` transaction-kind built against the live deployed contract, and the dev stand-in address as sender: it failed twice for real, informative reasons (the allow-list gap above, then a too-small escrow amount) and **succeeded** once both were fixed, returning real sponsored bytes + a digest. What could **not** be verified this session: the full HTTP round-trip through `/api/sponsor-transaction` starting from a genuinely-registered pending-tx, and `executeSponsoredTransaction` (which needs a real signature from a real `EnokiKeypair` — i.e. an actual signed-in Google/zkLogin browser session). The blocker wasn't the new code — it was that the only address available to test with, the old dev stand-in (`0xf93f...1efc05`), is now too low on testnet SUI to even complete `createOrder`'s existing, unchanged full-bytes-build step (needs ~2-4M MIST of gas headroom on top of the escrow amount), and the public testnet faucet was rate-limited (`429`) both times a top-up was attempted this session. In real operation this isn't expected to bite: a real owner's zkLogin address gets auto-fauceted on `/auth` before ever reaching `createOrder`. The final click-through (`/sign`'s "Approve & submit" button, end to end, in a real browser) remains genuinely unverifiable without a live Google session, same caveat as every other zkLogin UI flow in this project.

Also fixed this session, smaller but a real user-facing honesty bug: `/auth/page.tsx`'s `finishSignIn` called `/api/faucet` and threw away the result with `.catch(() => {})`, so an owner always saw "a small amount of testnet SUI has been requested for you" even when the faucet actually failed (a real `502`/rate-limit was hit earlier this session — see above) or was never called at all (no `telegramUserId` present). Fixed: the faucet response is now checked, with an honest failure message ("Funding request failed — the testnet faucet may be rate-limited...") shown instead of a false success claim, and the success message itself is now gated on a request having actually been attempted rather than always rendering. `dashboard/api/faucet/route.ts` was already correct (returns a real 502 + details on failure) — the bug was purely `/auth` discarding that response.
...
**One paragraph:** Giam Siap is an agentic B2B auto-buying engine. A restaurant owner tells a Telegram bot "lock RM500 for 50kg coffee at target RM10/kg." Funds get locked into a Sui Move escrow object (`ProcurementOrder`). A Hermes AI agent polls vendor price pages in the background; the moment a price meets the target, it builds and submits a Sui Programmable Transaction Block that atomically validates the price, releases escrow to the supplier, and refunds any excess — no human in the loop for settlement.
...
One terminology note before the stories: the original pitch language below says "USDC" and "testnet USDC" in places — that's from an earlier framing. Per the decision in the intro, **the actual unit locked and settled on-chain is testnet SUI**, not a custom USDC token. Ringgit figures (RM500, RM10/kg) are kept because that's how a Malaysian owner actually thinks and talks — the Telegram NL parser converts that into a SUI-denominated `target_price`/`amount` under the hood (§4.2's `createOrder` tool).

**Second terminology note, added 2026-09-04**: earlier drafts of this plan (and its code/prompts) used **USD** dollar figures ($500, $10/kg) as the illustrative currency. That's been switched to **Malaysian Ringgit (RM/MYR)** throughout — this system is being built for use in Malaysia, and the one real mock-vendor site that exists (`cooking-bistro.vercel.app`, §0.7) already prices everything in RM, not USD. This is a relabeling, not a logic change: every on-chain field/param that says "cents" (`target_price`, `price_cents`, `targetPriceCents`, `rate_mist_per_cent`) keeps its exact name — those names were already shared with teammates as the vendor interface spec (§4.2) and are treated as a generic "smallest currency unit," not literally US cents. "Cents" now means **sen** (1 RM = 100 sen). `rate_mist_per_cent`'s actual value is unchanged too — it was always an arbitrary demo-day rate, not a real exchange rate, so there's no real MYR/SUI market rate to look up.
...
|---|---|---|---|
| 1 | **Input** | Owner sends a natural-language command to the Telegram bot — e.g. *"Procure 50kg Coffee Beans with a max budget of RM500 at target price RM10/kg."* | §4.4 (Telegram gateway) — Hermes's own NL/tool-calling handles this, no custom parser needed |
...
| 4 | **Autonomous monitoring** | The agent fetches the vendor's price feed in the background on its own schedule — not waiting for the owner to ask — and posts status updates ("Monitoring active, vendor price RM12.00 exceeds target RM10.00") back into the Telegram chat. | §4.3 (orchestration loop / cron), §4.2 (`checkVendorPrice` tool) |
| 5 | **Outcome** | The moment the vendor's price meets or drops below RM10/kg, the agent builds and submits the `execute_order` PTB itself — no further owner approval needed, since the owner's own pre-set condition is what authorizes it. Telegram receives a confirmation with a Suiscan link to the settlement transaction. | §3 (`execute_order`), §6 (end-to-end diagram) |
...
| # | Step | What actually happens | Where it's built |
|---|---|---|---|
| 1 | **Input** | Supplier updates their catalog price — e.g. a flash sale drops coffee beans from RM12.00/kg to RM9.50/kg. This is entirely on the mock vendor site your teammates own; you only care that it's reachable at the agreed endpoint. | Teammates' deliverable (§0.7); the contract between you and them is §4.2's interface spec (`GET /api/price → {"price": <number>}`) |
...
public struct ProcurementOrder has key, store {
    ...
    target_price: u64,            // price per unit, in MYR CENTS (sen) — same unit the vendor's signature
...
public struct VendorRegistry has key {
    ...
    rate_mist_per_cent: u64,      // fixed demo-day MYR↔SUI rate (§4.5) — lives on-chain so create_order's
...
- `execute_order(order: &mut ProcurementOrder, cap: &AgentCap, registry: &VendorRegistry, price: u64, supplier: address, ts: u64, sig: vector<u8>, ctx)`
  Verifies `sig` against `registry.trusted_pubkey` over `(order.item_id, price, ts, supplier)` using `sui::ed25519::ed25519_verify` — this is what makes price *and* payee independently checkable instead of agent-asserted; note `price` here is in the same MYR-sen unit the vendor signed, matching `target_price`'s unit, so the comparison below needs no conversion. Also asserts `ts` is recent (rejects a replayed old signature). Then: asserts `price <= order.target_price`, asserts status is `Locked`, splits `(quantity * price) * registry.rate_mist_per_cent` MIST to `supplier` (the one place an RM→SUI conversion actually happens), refunds any remainder to `order.owner`, writes `order.supplier = option::some(supplier)`, sets status `Fulfilled`, emits `OrderFulfilled { order_id, price, supplier }`.
...
- `createOrder({ itemId, vendorUrls, targetPriceCents, quantity, paymentAmountMist })` — builds the `create_order` PTB. Note `targetPriceCents` is a direct MYR-sen conversion of what the owner said ("RM10/kg" → `1000`) — no exchange-rate math happens here, the contract's `VendorRegistry.rate_mist_per_cent` handles the only RM↔SUI conversion in the whole system (§3). **The owner signs this one, not the agent** — it's their money. The tool returns an unsigned transaction; the owner's browser signs it via the zkLogin flow in §4.5 (a real per-order signing step, not a server-side replay — see §4.5 for why). During early development, this can be signed by the throwaway stand-in keypair from §3's build checklist step 5 instead, per §7's build-order note.
- `executeOrder({ orderId, priceCents, supplierAddress, ts, sig })` — signs and submits with the `AgentCap` keypair, passing through the vendor's signed price quote unmodified (same MYR-sen unit end to end) for on-chain verification. Called only by the deterministic watcher (§4.3), never by the LLM directly.
...
│  0xab12..   0x9f3c...    RM10.00  RM12.00  🟡 MONITORING       │
│  0x7cd4..   0x9f3c...    RM10.00  RM9.50   🟢 EXECUTED         │
├─────────────────────────────────────────────────────────────┤
│  TRANSACTION LOG                                                │
│  ✓ Order 0x7cd4 executed at RM9.50/kg → suiscan.xyz/tx/abc123  │
...
```
Telegram (owner)
   │  "Lock RM500 for 50kg coffee at RM10/kg"
   ▼
...
Telegram: "Monitoring active..."                ▼
              ...
              tx log gets suiscan link             Executed at RM9.50/kg!"
                                                    + suiscan link
```
...
- [ ] The exact demo phrasing ("Procure 50kg Coffee Beans... target price RM10/kg") parses to the correct `{itemId, targetPrice, quantity}` **5 times in a row** — LLM parsing isn't fully deterministic, so test repeatedly, not once.
